...
module core.utils {
	export class Formula {
		private static _attackCache: any;
		private static _costCache: any;
		static getBallAttack(kId: string, level: number): number {
			var ballRes = dataMgr.sys_ballHash.getData(kId);
			if (level <= 0) return 0;
			if (level == 1) return ballRes.iAttack;
			var ratio = 0;
			...
			if (!this._attackCache) this._attackCache = {};
			var key = kId + "_" + level;
			if (this._attackCache[key]) return this._attackCache[key];
		...
		static getCost(kId: string, level: number): number {
			var ballRes = dataMgr.sys_ballHash.getData(kId);
			if (level <= 1) return ballRes.iOpenCost;
			var ratio = 1.1;
			if (ballRes.eType == SeEnumBalleType.MoFaQiu) {
				ratio = 1.15;
			} else if (ballRes.eType == SeEnumBalleType.XiaoQiu) {
			...
			var key = kId + "_" + level;
			if (this._costCache[key]) return this._costCache[key];
			var preKey = kId + "_" + (level - 1);
		...
		static getBallRealAttack(kid: string): number {
		...
		static StartCode = 65;	//A
		static numToShowStr(num: number): string {
